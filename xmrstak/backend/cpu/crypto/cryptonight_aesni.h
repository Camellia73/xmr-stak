...
extern "C"
{
	...
	extern void do_blake_hash(const void* input, uint32_t len, char* output);
...
#define CN_FINALIZE(n) \
	...
	{ \
		do_blake_hash(ctx[n]->hash_state, 200, (char*)output + 32 * n); \
	} \
	else \
	{ \
...
struct Cryptonight_hash_asm<1, asm_version>
{
	...
	static void hash(const void* input, size_t len, void* output, cryptonight_ctx** ctx)
	{
		...
		if (ALGO == cryptonight_dark)
		{
			do_blake_hash(ctx[0]->hash_state, 200, (char*)output);
		}
		else
		{
...
struct Cryptonight_hash_asm<2, 0>
{
	...
	static void hash(const void* input, size_t len, void* output, cryptonight_ctx** ctx)
	{
		...
		for(size_t i = 0; i < N; ++i)
		{
			...
			if (ALGO == cryptonight_dark)
			{
				do_blake_hash(ctx[i]->hash_state, 200, (char*)output + 32 * i);
			}
			else
			{
